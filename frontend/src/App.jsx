...
export default function App() {
  ...
  useGSAP(() => {
    // 1. 横向滚动 GSAP ScrollTrigger Timeline (1屏时间留给首屏拉近，3屏时间用于横移)
    const horizontalTween = gsap.timeline({
      scrollTrigger: {
        ...
        scrub: 0.5,
        start: 'top top',
        end: () => `+=${window.innerWidth * 2.5}`, // 滚动深度为2.5屏宽度，加快滚动速度
        invalidateOnRefresh: true,
      }
    });

    horizontalTween.to({}, { duration: 0.5 });
    ...
    // 2. 个人经历与核心指标 (Experience) 页面入场动画 (基于横向平移触发)
    if (experienceRef.current) {
      gsap.set('.animate-profile-card', { x: -150, opacity: 0 });
      gsap.set('.animate-bio-title', { y: -60, opacity: 0 });
      gsap.set('.animate-bio-subtitle', { y: -60, opacity: 0 });
      gsap.set('.animate-filter-btn', { x: 100, y: -100, opacity: 0 });
      gsap.set('.animate-gallery-wrapper', { x: 200, opacity: 0 });

      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: experienceRef.current,
          containerAnimation: horizontalTween,
          start: 'left 65%', // 当Section左侧划入屏幕 65% 时触发
  ...
  useEffect(() => {
    ...
    // Smooth scroll for anchor hash links (nav items mapped to vertical ScrollTrigger depth)
    const handleAnchorClick = (e) => {
      const targetLink = e.target.closest('a[href^="#"]');
      if (targetLink) {
        e.preventDefault();
        const H = window.innerWidth * 2.5;
        if (id === '#hero') {
          lenis.scrollTo(0, { duration: 1.2 });
          setActiveNavIndex(0);
        } else if (id === '#experience') {
          lenis.scrollTo(H * (1.5 / 2.5), { duration: 1.2 });
          setActiveNavIndex(1);
        } else if (id === '#competencies') {
          lenis.scrollTo(H, { duration: 1.2 });
          setActiveNavIndex(2);
        }
      }
    };
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0C] text-white selection:bg-[#FF6B00] selection:text-white relative font-sans overflow-x-hidden">
            ...
            <div className="hidden md:flex items-center text-xs font-mono tracking-wider">
              <GooeyNav
                ...
                onActiveIndexChange={(index) => {
                  ...
                  if (lenisRef.current) {
                    const H = window.innerWidth * 2.5;
                    const targetScroll = index === 0 ? 0 : index === 1 ? H * (1.5 / 2.5) : H;
      ...
      {/* 横向滚动房间容器 */}
      <div className="horizontal-scroll-container relative z-10">
        {/* ==================== Room 1: 全屏首页 HERO 区域 ==================== */}
        ...
        {/* ==================== Room 3: 个人优势与能力看板（CORE COMPETENCIES） ==================== */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-[#121215] border border-neutral-800 hover:border-[#FF6B00]/40 rounded-2xl p-6 transition-all duration-300 flex flex-col justify-between group">
                <div className="space-y-4">
                  ...
                  <p className="text-neutral-400 text-xs leading-relaxed font-light">
                    熟练掌握 Java 集合底层数据结构与 JVM 内存管理、垃圾回收算法及双亲隔离模型。深入探究线程池、ThreadLocal、CAS 原子指令、AQS 独占共享以及 JMM 内存模型。
                  </p>
                </div>
